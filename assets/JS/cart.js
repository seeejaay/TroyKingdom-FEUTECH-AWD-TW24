document.addEventListener('DOMContentLoaded', function() {
    ...
    const cartButton = document.getElementById('_btnCartL'); // Cart button
    ...
    addToCartButtons.forEach(button => {
      button.addEventListener('click', function() {
        ...
        const productPrice = parseFloat(productCard.querySelector('.pricing-card-title').innerText.replace(/[^\d.-]/g, '')); // Extract price as float
  
        // If the item is already in the cart, increment its quantity
        if (cartItems.hasOwnProperty(productId)) {
          cartItems[productId].quantity++;
          const existingCartItem = cartItemsList.querySelector(`[data-id="${productId}"]`);
          existingCartItem.querySelector('.quantity').value = cartItems[productId].quantity;
          updateTotalPrice();
        } else {
    ...
    function updateTotalPrice() {
      totalPrice = Object.values(cartItems).reduce((total, item) => total + (item.price * item.quantity), 0);
      totalPriceElement.innerText = `₱${totalPrice.toFixed(2)}`;
    }
  });
